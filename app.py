...
def process_data(comments_df, price_df, method="LLM法", lag_days=1):
    ...
    if method == "词典法":
        ...
        label_col = "lexicon_sentiment_label" if "lexicon_sentiment_label" in filtered_comments.columns else None
    elif method == "LLM法":
        score_col = "llm_sentiment_score_new"
        label_col = "llm_sentiment_label_new"  # 直接使用CSV中已有的标签
    elif method == "集成法":
        score_col = "ensemble_sentiment_score_new"
        label_col = "ensemble_sentiment_label_new" if "ensemble_sentiment_label_new" in filtered_comments.columns else None
    else:
        method = "LLM法"
        score_col = "llm_sentiment_score_new"
    ...
    daily_sentiment = filtered_comments.groupby(filtered_comments['post_publish_time'].dt.date).agg({
        ...
        label_col: lambda x: x.value_counts() if label_col else None
    }).reset_index()
...
# 绘制对比表格：直接使用CSV中已有的标签字段
def plot_sentiment_table(comments_df):
    stats_data = []
    # 1. 词典法（若有标签字段）
    if "lexicon_sentiment_label" in comments_df.columns:
        lexicon_mean = comments_df["lexicon_sentiment"].mean()
        lexicon_std = comments_df["lexicon_sentiment"].std()
        lexicon_pos = (comments_df["lexicon_sentiment_label"] == "积极").sum() / len(comments_df) * 100
        lexicon_neu = (comments_df["lexicon_sentiment_label"] == "中性").sum() / len(comments_df) * 100
        lexicon_neg = (comments_df["lexicon_sentiment_label"] == "消极").sum() / len(comments_df) * 100
    else:
        lexicon_mean = comments_df["lexicon_sentiment"].mean()
        lexicon_std = comments_df["lexicon_sentiment"].std()
        lexicon_pos = 0
    ...
    # 2. LLM法（使用CSV中已有的llm_sentiment_label_new）
    ...
    # 3. 集成法（若有标签字段）
    if "ensemble_sentiment_label_new" in comments_df.columns:
        ensemble_mean = comments_df["ensemble_sentiment_score_new"].mean()
        ensemble_std = comments_df["ensemble_sentiment_score_new"].std()
        ensemble_pos = (comments_df["ensemble_sentiment_label_new"] == "积极").sum() / len(comments_df) * 100
        ensemble_neu = (comments_df["ensemble_sentiment_label_new"] == "中性").sum() / len(comments_df) * 100
        ensemble_neg = (comments_df["ensemble_sentiment_label_new"] == "消极").sum() / len(comments_df) * 100
    else:
        ensemble_mean = comments_df["ensemble_sentiment_score_new"].mean()
        ensemble_std = comments_df["ensemble_sentiment_score_new"].std()
        ensemble_pos = "-"
...
try:
    ...
    # 直接使用CSV中已有的标签字段
    sentiment_counts = filtered_comments[label_col].value_counts() if label_col else pd.Series()
    ...
    # 4. 评论示例
    st.subheader('评论示例')
    sample_cols = ['post_publish_time', 'combined_text']
    if label_col:
        sample_cols.append(label_col)
    sample_cols.append(score_col)
    sample_comments = filtered_comments[sample_cols].dropna()
    if len(sample_comments) > 0:
        st.dataframe(sample_comments.sample(min(5, len(sample_comments))), use_container_width=True)
    else:
        st.write('暂无有效评论示例')
